...
def test_postprocessor_connects_raw_obstacles_to_tof_output():
    ...
    assert parameters['distance_max'] > 0.0
...
def test_forward_distance_uses_sensor_origin_and_includes_bounds():
    points = np.asarray([
        [0.25, 0.0, 0.2],
        [0.26, 0.0, 0.2],
        [0.80, 0.0, 0.2],
    ...
    mask = forward_distance_filter_mask(
        points, origin_x=0.26, distance_max=1.0)

    assert mask.tolist() == [False, True, True, True, False]


@pytest.mark.parametrize('distance_max', [0.0, -1.0, np.inf, np.nan])
def test_forward_distance_rejects_invalid_limit(distance_max):
    points = np.empty((0, 3), dtype=np.float32)

    with pytest.raises(ValueError):
        forward_distance_filter_mask(points, 0.26, distance_max)
