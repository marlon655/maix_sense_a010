...
// POSIX headers (place them here so close(), read(), write(), tcgetattr, etc. are declared)
#ifndef WIN32
...
#include <string.h>
#else
// Windows headers would go here if Windows support is added
#endif



#ifndef WIN32
#define MS_A010_DEFAULT_SERIAL_DEV_PATH "/dev/tof"
#else
...
Serial::Serial(const std::string &dev_path) : dev_path(dev_path) {
  ...
  this->priv = this->configure_serial();
  if (this->priv == nullptr) {
    std::cerr << "Error: failed to configure serial device (" << this->dev_path << ")" << std::endl;
    return;
  }
}
...
Serial::~Serial() {
  if (this->priv) {
    ...
    if (fd >= 0) {
      // Use ::close para evitar ambiguidade de nomes
      ::close(fd);
    }
...
#else
Serial::~Serial() {
  // Windows-specific cleanup (if you later implement Windows support)
}
#endif
...
static inline int serial_setup(int serial_port, unsigned int baudrate) {
  ...
  // use macros B115200 etc. map from integer
